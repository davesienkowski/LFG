...
**Choice:** Single fetch + pure helper (D3-01/02). **Notes:** Cells already need all vote rows, so tallies are free in JS — no 2nd query. The lexicographic tie-break (yes↓, ifneedbe↓, date↑) is the highest-risk correctness surface; a pure function makes it unit-testable. SQL GROUP BY deferred as a scaling alternative.
...
**Choice:** Semantic `<table>` (D3-04). **Notes:** Correct accessible structure for a 2-D grid — screen readers announce row/column headers. Cells reuse `STATE_META` icon+label (color never the only signal).
...
**Choice:** Client-side filter-hide (D3-06). **Notes:** Organizer's real question is "who's available on date X" — hiding non-matches answers it directly. All data already client-side; no round-trip. Zero-match shows an explicit message.
...
**Choice:** "Best" pill + emerald tint (D3-07). **Notes:** The "Best" text label carries the signal so the highlight isn't color-only. All co-leaders (tied on yes then if-need-be) badged; none when no date has ≥1 yes.
