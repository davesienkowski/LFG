...
- [x] **RESP-01**: On the admin view, the organizer can see which invited people have not yet responded (invited emails shown with a responded / not-responded status, matched to participants).
- [x] **RESP-02**: The organizer can send a one-click "nudge" reminder email to the non-respondents, containing the participant link. *(Action + fallback shipped; live email delivery an accepted limitation — see Traceability note.)*
- [x] **RESP-03**: When the organizer sends invitation emails, the recipients are persisted, so respondent tracking has a source of truth (v1.0 sent invites without recording who received them).

### Scheduling Controls

- [x] **DEAD-01**: The organizer can set an optional voting deadline on a poll; once the deadline has passed the poll is closed to further voting (the vote form becomes read-only, consistent with FNL-02). Deadline expiry is evaluated lazily on poll access — no scheduled/cron job (respects the Vercel Hobby cron limitation documented in v1.0 Out of Scope).
- [x] **ORG-01**: The organizer can add and edit their own availability row from the admin view, without using the participant link, and it appears in the results grid like any other participant.
...
| RESP-02 | Phase 7 | Complete (email delivery is an accepted current limitation — see note) |
| DEAD-01 | Phase 8 | Complete & verified |
| ORG-01 | Phase 8 | Complete & verified |

> **RESP-02 email-delivery note (2026-09-07):** The nudge action, non-respondent targeting, and graceful copy-link fallback are shipped and verified in code. Actual outbound email delivery from this project is a **known and accepted limitation** at milestone close — participants can still open the poll, respond, and choose. No code gap; revisit if/when email sending is re-enabled.
